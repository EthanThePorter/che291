...
def fugacity(mf, W):
    ...
    # Equation 32
    a = 0.42747 * R ** 2 * CPT[:, 1] ** 2 / CPT[:, 0] * (
                1 + (0.48508 + 1.55171 * CPT[:, 2] - 0.15613 * CPT[:, 2] ** 2) * (1 - np.sqrt((T + 273.15) / CPT[:, 1]))) ** 2
    # Equation 33
    b = 0.08664 * R * CPT[:, 1] / CPT[:, 0]
    ...
    # Equation 36
    phi = np.exp(B / Bm * (Zm - 1) - np.log(Zm - Bm) - Am / Bm * (2 * np.dot(mf, Aij)/Am - B/Bm)*np.log(1 + Bm / Zm))
...
Results11.to_excel('ChE291_E6_Task2.xlsx')
