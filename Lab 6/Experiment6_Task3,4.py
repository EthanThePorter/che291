...
def DewPointEquations(m):
    fp = np.zeros(n + 1)
    x = m[0:n]  # Mole fractions of liquid phase.
    y = z  # Mole fraction of vapor phase.
    T = m[n]  # Ratio of vaporization.
...
for i in range(Nn):
    print('Pop', Pop[i])
    P = Pop[i]
    Tod[i] = 60 + i * 4  # Guessed bubble point in Celsius
    mod = [0.1, 0.1, 0.1, 0.35, 0.35, Tod[i]]
    md = optimize.fsolve(DewPointEquations, mod, xtol=1e-6)
...
df['Tdp'] = Tdp
df['Tbp'] = Tbp
df.to_excel('CHE 291 L6 3,4 Results.xlsx', index=False)
